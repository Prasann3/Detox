...
Auto-Blur Toxic CommentsToxic content is blurred by default so that you aren’t exposed to harmful language unintentionally.

Unblur and Reblur ToggleUsers can unblur any blurred comment with a click, and also reblur it at any time using the added reblur button.

Multi-Language SupportThe extension first sends text to an external API for language detection and translation to English, ensuring toxic content is caught in any language.
...
Third-party APIs are used only for language detection and translation, not for storing or tracking content.
